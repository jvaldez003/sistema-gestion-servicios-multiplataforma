...
class _PersonalDataScreenState extends ConsumerState<PersonalDataScreen> {
  ...
  Future<void> _editPhotoUrl(String userId, String currentUrl) async {
    ...
    setState(() => _isSaving = true);
    try {
      final storageRef = FirebaseStorage.instance.ref('users/$userId/profile.jpg');
      await storageRef.putFile(File(picked.path));
      final downloadUrl = await storageRef.getDownloadURL();
      await ref.read(userRepositoryProvider).updateUserProfile(userId, photoUrl: downloadUrl);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
                  ...
                  child: Column(
                    children: [
                      _buildEditableField(
                        ...
                        value: userData.name?.isNotEmpty == true ? userData.name! : 'No configurado',
                        onEdit: () => _editField(
                          title: 'Nombre',
                          currentValue: userData.name ?? '',
                          onSave: (val) => ref.read(userRepositoryProvider).updateUserProfile(userData.id, name: val),
                        ),
                      ),
                      ...
                      _buildEditableField(
                        ...
                        value: userData.email.isNotEmpty ? userData.email : 'No configurado',
                        onEdit: () => _editField(
                          title: 'Correo',
                          currentValue: userData.email,
                          keyboardType: TextInputType.emailAddress,
                          onSave: (val) => ref.read(userRepositoryProvider).updateUserProfile(userData.id, email: val),
                        ),
                      ),
                      Divider(height: 1, indent: 16, endIndent: 16, color: Theme.of(context).dividerColor),
                      _buildEditableField(
                        ...
                        label: 'Teléfono',
                        value: userData.phoneNumber?.isNotEmpty == true ? userData.phoneNumber! : 'No configurado',
                        onEdit: () => _editField(
                          title: 'Teléfono',
                          currentValue: userData.phoneNumber ?? '',
                          keyboardType: TextInputType.phone,
                          onSave: (val) => ref.read(userRepositoryProvider).updateUserProfile(userData.id, phoneNumber: val),
                        ),
                        isLast: true,
                      ),
                    ],
              ...
              SliverToBoxAdapter(
                child: Padding(
                  ...
                  child: Row(
                    children: [
                      Expanded(
                        child: _buildStatCard(context, isDark: isDark,
                          icon: Icons.stars_rounded, color: const Color(0xFFF59E0B),
                          label: 'Puntos', value: '${userData.points}'),
                      ),
                      const SizedBox(width: AppSpacing.md),
                      Expanded(
                        child: _buildStatCard(context, isDark: isDark,
                          icon: Icons.favorite_outline_rounded, color: const Color(0xFFEC4899),
                          label: 'Favoritos', value: '${userData.favoriteIds.length}'),
                      ),
                      const SizedBox(width: AppSpacing.md),
                      Expanded(
                        child: _buildStatCard(context, isDark: isDark,
